  ...
  onSubmitBooking() {
    ...
    this.http.post<any>(`${this.baseUrl}/bookings`, bookingPayload, { headers }).subscribe({
      next: (bookingRes) => {
        ...
        this.router.navigate(['/parent/payment', this.createdBookingId]);
      },
      error: (bookingErr) => {
        this.isLoading.set(false);
        ...
        next: (paymentStatusRes) => {
          ...
          if (paymentStatusRes.status === 'Completed' || paymentStatusRes.status === 1) {
            ...
            this.errorMessage.set(
              'فشلت عملية الدفع عبر Paymob، يرجى إعادة المحاولة أو مراجعة الرصيد.',
            );
          }
        },
        error: (err) =>
          console.error('جاري انتظار الـ Webhook المالي لتأكيد الحجز من السيرفر...', err),
      });
    }, 2000);
  }
