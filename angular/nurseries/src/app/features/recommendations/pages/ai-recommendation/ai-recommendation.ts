...
export class AiRecommendation {
  ...
  getRecommendations() {
    ...
    if (!auth.isValidParent) {
      ...
      this.errorMessage.set(`⚠️ عذراً، ميزة الترشيحات الذكية متاحة فقط لحسابات أولياء الأمور (Parent).`);
      return;
    }
    ...
    this.http.post<any>(this.apiUrl, aiPayload, { headers: auth.headers }).subscribe({
      next: (res) => {
        ...
        if (Array.isArray(recommendedData) && recommendedData.length > 0) {
          ...
          this.errorMessage.set('لم يعثر الذكاء الاصطناعي على حواضن طبية تطابق هذه المعايير الدقيقة حالياً.');
        }
      },
      error: (err) => {
        this.isLoading.set(false);
