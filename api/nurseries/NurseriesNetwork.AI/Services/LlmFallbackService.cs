...
public class LlmFallbackService : ILlmService
{
    ...
    public async Task<string> GetChatResponseAsync(string systemPrompt, string userMessage)
    {
        try
        {
            return await _gemini.GetChatResponseAsync(systemPrompt, userMessage);
        }
        catch (GeminiRateLimitException)
        {
            _logger.LogWarning("Gemini rate-limited. Falling back to Grok for GetChatResponseAsync.");
            return await _grok.GetChatResponseAsync(systemPrompt, userMessage);
        }
    }
    ...
    public async Task<IntentClassificationResult> ClassifyIntentAsync(string userMessage)
    {
        try
        {
            return await _gemini.ClassifyIntentAsync(userMessage);
        }
        catch (GeminiRateLimitException)
        {
            _logger.LogWarning("Gemini rate-limited. Falling back to Grok for ClassifyIntentAsync.");
            return await _grok.ClassifyIntentAsync(userMessage);
        }
    }
        ...
        string userMessage, string conversationContext = "")
    {
        try
        {
            return await _gemini.GetFunctionCallAsync(userMessage, conversationContext);
        }
        catch (GeminiRateLimitException)
        {
            _logger.LogWarning("Gemini rate-limited. Falling back to Grok for GetFunctionCallAsync.");
            return await _grok.GetFunctionCallAsync(userMessage, conversationContext);
        }
    }
